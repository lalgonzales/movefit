...
def test_trends_with_window_preserves_trend_dir(
    client: TestClient, metric: str, window: str
):
    # Reset DB
    with Session(client.app.state._test_engine) as session:
        session.execute(text("DELETE FROM measurement"))
    ...
    response = client.get(
        f"/trends?metric={metric}&window={window}&from=2026-03-01T00:00:00Z&to=2026-04-30T23:59:59Z"
    )
    assert response.status_code == 200
    data = response.json()
...
def test_bulk_import_xlsx(client: TestClient, tmp_path):
    ...
    spanish_data = [
        {
            "Tiempo de medición": "2026-03-22T08:00:00Z",
            "Mac dispositivo": "DD:DD:DD:DD:DD:DD",
            "Nombre dispositivo": "Scale D",
            "Peso(lb)": 180.0,
            "weight_kg": 81.6,
            "Grasa corporal(%)": 22.1,
            "bmi": 26.4,
        ...
        {
            "Tiempo de medición": "2026-03-23T08:00:00Z",
            "Mac dispositivo": "EE:EE:EE:EE:EE:EE",
            "Nombre dispositivo": "Scale E",
            "Peso(lb)": 190.0,
            "weight_kg": 86.2,
            "Grasa corporal(%)": 23.5,
            "bmi": 27.0,
